<template>
  <div class="app-container calendar-list-container">
    ...
    <el-table size="small" :data="list" v-loading="listLoading" element-loading-text="正在查询中。。。" border fit
      ...
      <el-table-column align="center" min-width="100px" label="订单状态" prop="orderStatus" :formatter="formateOrderStatus">>
      </el-table-column>

      <el-table-column align="center" min-width="100px" label="是否删除" prop="isDelete">
        <template slot-scope="scope">
          <el-tag :type="scope.row.deleted ? 'danger' : 'success' ">{{scope.row.deleted ? '已删除' : '未删除'}}</el-tag>
        </template>
      </el-table-column>
...
<script>
  import {listOrder, updateOrder, formateOrderStatus} from '@/api/order'
  ...
  export default {
    ...
    methods: {
      ...
      formateOrderStatus(row, column) {
        return formateOrderStatus(row, column);
      },
      handleFilter() {
